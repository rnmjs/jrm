...
describe("Runtime", () => {
  // eslint-disable-next-line @typescript-eslint/init-declarations -- for test
  ...
  describe("getRemoteVersions", () => {
    it("should filter out prerelease versions", async () => {
      const testRuntimeWithPrerelease = new TestRuntime();
      vi.mocked(fs.readdir).mockResolvedValue([]);

      await testRuntimeWithPrerelease.install("^2.0.0");
